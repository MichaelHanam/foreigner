...
func pick_up(player: Player, entity: NPC):
	if carried_object:
		return
	
	carried_object = entity
	carried_object.carriable.on_picked_up(player, carried_object)

func place(player: Player):
	carried_object.carriable.on_placed(player, carried_object)
	carried_object = null
...
func physics_update(player: Player):
	if carried_object:
		...
		carried_object.carriable.set_is_placable(can_place(player))
